...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm=False):
        ...
        if norm:
            self.norm = nn.InstanceNorm2d(planes*self.expansion, affine=True)
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, loss, block, layers,
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], norm=True)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, norm=True)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, norm=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, norm=False):
        downsample = None
...
def resnet50_ifn(num_classes=None, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
